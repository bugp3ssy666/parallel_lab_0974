...
public:
    ...
    // 一次性传递
    void LoadAllOrderedValuesToGPU();
    void FreeGlobalBuffers();
    char *d_letters_all = nullptr, *d_digits_all = nullptr, *d_symbols_all = nullptr;
    int *d_letter_offsets = nullptr, *d_digit_offsets = nullptr, *d_symbol_offsets = nullptr;
    int total_letter_count = 0, total_digit_count = 0, total_symbol_count = 0;

    std::vector<int> h_letter_offsets_gpu;
    std::vector<int> h_digit_offsets_gpu;
    std::vector<int> h_symbol_offsets_gpu;
};