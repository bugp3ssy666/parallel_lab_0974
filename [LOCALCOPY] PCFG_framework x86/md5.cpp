...
Byte *SIMDStringProcess(string *inputs, int *n_byte, int guess_num, int &max_byte)
{
	...
	// ALIGNMENT： SSE SIMD 向量指令处理数据的对齐要求是 *16 字节对齐*
	// 已经求得所有消息中最大的 paddingByte 数组长度，根据这个长度进行内存对齐即可
	constexpr int ALIGNMENT = 16;
	Byte *paddedData = (Byte *)_mm_malloc(maxPaddedLength * guess_num, ALIGNMENT);
...
void SIMDMD5Hash(string *inputs, bit32 *state)
{
	...
	Byte *paddedData;
	paddedData = SIMDStringProcess(inputs, messageLengths, PARA_NUM, maxByte);
	...
	for (int i = 0; i < maxBlocks; i += 1) {
		...
		__m128i activeMask = _mm_set_epi32(maskArray[3], maskArray[2], maskArray[1], maskArray[0]);
